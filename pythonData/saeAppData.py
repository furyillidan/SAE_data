...
#url="https://www.gamertb.com/arena/a198/"
...
# test data
herolist = {
       "ge" :    [{"name" : "盧修斯" , "positioning" : "坦克", "intro" : "前期到後期萬用坦克，主動技團隊護盾很實用。", "firstdraw" : "★★★★★"} , 
       ...
       "blood" : [{"name" : "布魯特斯" , "positioning" : "坦克、輸出", "intro" : "全遊戲大家最想抽到的角色，無敵狀態鎖血非常強勢、主動技傷害高。", "firstdraw" : "★★★★★"} , 
       ...
       "green" : [{"name" : "妮莫拉" , "positioning" : "輔助、控場", "intro" : "目前最優秀的補師，另外可以善用魅惑來抓住對面的輔助、控場。", "firstdraw" : "★★★★★"} , 
       ...
       "dead" : [{"name" : "雪米拉" , "positioning" : "控場、坦克、輸出", "intro" : "國外認定最強英雄，能坦能控能輸出（lv161後）。", "firstdraw" : "★★★★★"} , 
       ...
       "helfgod" : [{"name" : "阿塔利婭" , "positioning" : "控場、輸出", "intro" : "開場能封鎖對面後排，傷害高，整體強度算是僅次於雪米拉的英雄，課長專用。", "firstdraw" : "★★（無課）,★★★★★（課金）"} , 
       ...
       "demon" : [{"name" : "梅希拉" , "positioning" : "輔助、控場、輸出", "intro" : "群體魅惑非常強勢、給前排加速，外加惡魔陣營給的 30% 防禦，課長專用。", "firstdraw" : "★★★（無課）,★★★★★（課金）"} , 
       ...
       "netherguest" : [{"name" : "亞瑟" , "positioning" : "坦克", "intro" : "群體魅惑非常強勢、給前排加速，外加惡魔陣營給的 30% 防禦，課長專用。", "firstdraw" : "★★★（無課）,★★★★（課金）"}],
}

heroIntro = {
       "ge" :    [ {"name" : "塞西莉亞" , "type" : "敏捷型", "level" : "神話"},
               {"name" : "裡格比" , "type" : "力量型", "level" : "神話"},
               {"name" : "羅莎琳" , "type" : "智慧型", "level" : "神話"},
               {"name" : "格溫妮絲" , "type" : "力量型", "level" : "神話"},
               {"name" : "羅萬" , "type" : "智慧型", "level" : "神話"},
               {"name" : "亨德里克" , "type" : "力量型", "level" : "神話"},
               {"name" : "瑟恩" , "type" : "敏捷型", "level" : "神話"},
               {"name" : "盧修斯" , "type" : "力量型", "level" : "神話"} ,
               {"name" : "弗克斯" , "type" : "敏捷型", "level" : "神話"},
               {"name" : "雷恩" , "type" : "敏捷型", "level" : "神話"},
               {"name" : "貝琳達" , "type" : "智慧型", "level" : "神話"},
               {"name" : "愛絲特瑞達" , "type" : "力量型", "level" : "神話"},
               {"name" : "米瑞兒" , "type" : "智慧型", "level" : "史詩+"},
               {"name" : "莫韋斯" , "type" : "敏捷型", "level" : "史詩+"},
               {"name" : "安吉洛" , "type" : "智慧型", "level" : "史詩+"},
               {"name" : "霍根" , "type" : "力量型", "level" : "史詩+"},
               {"name" : "皇家衛士" , "type" : "力量型", "level" : "普通"},
               {"name" : "皇家牧師" , "type" : "智慧型", "level" : "普通"}],

       "blood" :    [ {"name" : "塞西莉亞" , "type" : "敏捷型", "level" : "神話"},
               {"name" : "裡格比" , "type" : "力量型", "level" : "神話"},
               {"name" : "羅莎琳" , "type" : "智慧型", "level" : "神話"},
               {"name" : "格溫妮絲" , "type" : "力量型", "level" : "神話"},
               {"name" : "羅萬" , "type" : "智慧型", "level" : "神話"},
       ...
       "netherguest" : [{"name" : "亞瑟" , "type" : "坦克", "level" : ""}],
} 
...
allHeroList = []
heroIntroModelList = []
url4="https://www.gamertb.com/arena/hero-list-4/"
#抓取每個勢力英雄清單
def allHeroData(heroListUrl):
 ...
 img_links4 = soup4.find_all(["table","td"])
 ...
 for t in range(int(dataCount)):
    #print (t + 1)
    # print(heroDataList4[t])
    # print(heroDataList4[t+1])
    # print(heroDataList4[t+2])
    heroDetailDic['name'] = heroDataList4[t]
    heroDetailDic['type'] = heroDataList4[t +1]
    heroDetailDic['level'] = heroDataList4[t +2]
    # print(heroDetailDic['name'])
    # print(heroDetailDic['type'])
    # print(heroDetailDic['level'])
    #print(heroDetailDic)
    heroIntroModelList.append(heroDetailDic)
    # print("===",heroIntroModelList,"===")
 #return heroIntroModelList
    #print(len(heroIntroModelList))
...
print(testUse)
bloodDic = {}
bloodDic['blood'] = heroIntroModelList
allHeroList.append(bloodDic)
...
@app.route('/heroIntro')
def intro():
  return jsonify(heroIntro)

@app.route('/testlist')
def test():
  return jsonify(allHeroList)
